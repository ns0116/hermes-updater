...
**クラウドセッションでの作業範囲**: このアプリはWindowsネイティブのシステムトレイ常駐アプリであり、実際のHermes Agentインストール・実プロセス・タスクスケジューラを前提とした操作（上記`SEMI_AUTO`/`禁止`項目）はクラウドセッションでは実行不可（対象システムが存在しない）。クラウドではコード変更・`pytest`実行・ドキュメント更新に限定する。
